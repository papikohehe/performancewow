...
**กติกาการนับชั่วโมง:** จะนับชั่วโมง **เฉพาะช่วงที่มี `Processed Count` > {HOUR_COUNT_THRESHOLD}** เท่านั้น
"""
)

with st.sidebar:
    st.header("⚙️ การตั้งค่า")
    st.caption(f"เกณฑ์การนับชั่วโมง: `Processed Count` > {HOUR_COUNT_THRESHOLD}")
    ...
    for d in dates:
        for h in hours:
            for eid, nm in employees:
                # Generate counts that often exceed the 100-threshold so the sample showcases hour counting
                if rng.random() < 0.8:
                    cnt = int(rng.poisson(130))  # centered well above 100
                else:
                    cnt = 0
...
# --- Aggregates with fixed threshold rule (>100) ---
emp_hourly, emp_daily, emp_summary = compute_aggregates(
    df, processed_threshold=HOUR_COUNT_THRESHOLD
)
...
with tab1:
    ...
    st.caption(f"ใช้เกณฑ์ชั่วโมงทำงานขั้นต่ำ **{min_hours_threshold}** ชั่วโมงสำหรับ WPH (ชั่วโมงจะถูกนับเมื่อ `Processed Count` > {HOUR_COUNT_THRESHOLD})")
